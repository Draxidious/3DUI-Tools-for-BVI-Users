...
public class RaycastAnnouncer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // --- Perform SphereCast (all hits + sorting + ignore) ---
        ...
        GameObject objectJustHit = null;

        // Pick the first hit that's not in the ignore list
        foreach (var h in allHits)
        {
            if (!ignoreList.Contains(h.collider.gameObject))
            {
                hitDetected = true;
                hitInfo = h;
                objectJustHit = h.collider.gameObject;
                break;
            }
        }

        wasHitLastFrame = hitDetected;
        if (hitDetected) lastHitInfo = hitInfo;
        ...
        if (hitDetected)
        {
            ...
            if (!isIgnored)
            {
                ...
                if (generalCooldownElapsed)
                {
                    ...
                    else if (Time.time >= lastAnnouncementTime + repeatAnnounceDelay)
                    {
                        // --- Case 2: Same object after repeat delay ---
                        announce = true;
                    }
                    ...
                    if (announce)
                    {
                        ...
                        string clean = CleanName(currentlyHitObject.name);
                        float distanceVal = hitInfo.distance;
                        messageToSpeak = $"{clean} is {distanceVal:F1} meters away";

                        Debug.Log($"Announcing: \"{messageToSpeak}\"");
                        ttsSpeaker.SpeakQueued(messageToSpeak);

                        lastAnnouncedObject = currentlyHitObject;
    ...
    private void OnDrawGizmos()
    {
        ...
        Vector3 endPosition = origin + direction * Mathf.Max(0, distance);
        if (drawHitSphere)
